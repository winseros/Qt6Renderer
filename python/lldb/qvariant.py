...
class QVariantSynth(AbstractSynth):
    ...
    def _void_star(self, d: SBValue):
        t = self._valobj.GetFrame().GetModule().FindFirstType('void').GetPointerType()
    ...
    def _internal(self, d: SBValue, basic_type: int):
        t = self._valobj.GetTarget().GetBasicType(basic_type)
        value = self._valobj.CreateValueFromAddress(self.PROP_VALUE, d.load_addr, t)
        self._values = [value]
    ...
    def _external(self, d: SBValue, p_data: int):
        ...
        type_name = self._read_cstr(self._valobj.GetProcess(), p_type_name)
        t = self._valobj.GetFrame().GetModule().FindFirstType(type_name)
