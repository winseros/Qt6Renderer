...
public class GDBRegistrar {
    public static void register(@NotNull IdeaPluginDescriptor plugin,
        ...
        Path path = plugin.getPluginPath().resolve("python");
...
//        debuggerDriver.executeInterpreterCommand(threadId, frameIndex, "python import qt6renderer_lookup");
//        debuggerDriver.executeInterpreterCommand(threadId, frameIndex, "python gdb.pretty_printers.append(qt6renderer_lookup.pp_lookup)");

        debuggerDriver.executeInterpreterCommand(threadId, frameIndex, String.format("python sys.path.append(\"%s\")", escapedPath));
        debuggerDriver.executeInterpreterCommand(threadId, frameIndex, "python import frontend.gdb.lookup");
        debuggerDriver.executeInterpreterCommand(threadId, frameIndex, "python gdb.pretty_printers.append(frontend.gdb.lookup.print)");
    }
}
